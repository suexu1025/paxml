...
_MLPERF_GPT3_START_TRACE_STEP = flags.DEFINE_integer(
    'mlperf_gpt3_start_trace_step',
    100,
    'Start step of profiler trace.',
)
...
class MLPerfTrainProgram(programs.SingleTaskTrainProgram):
  ...
  def run(
    ...
    if self._is_compiled:
      if not self._start_trace_step:
        # start calculating self._start_trace_step
        #   from loading pretrained checkpointing
        # checkpoint loading will be finished after _is_compile
        self._start_trace_step = (
            _MLPERF_GPT3_START_TRACE_STEP.value + train_step
        )
        logging.info(
            '[trace]: To start trace at step=%d', self._start_trace_step
        )

      if (
          _MLPERF_GPT3_TRACE_STEP.value
          and os.getenv('MEGASCALE_SLICE_ID', None) == '0'
          and os.getenv('TPU_WORKER_ID', None) == '0'
          and os.getenv('GS_PREFIX', None)
      ):
        if train_step == self._start_trace_step:
          logging.info('[trace]: Start trace at step=%d', train_step)
          jax.profiler.start_trace(f'gs://{os.getenv("GS_PREFIX")}')
        if train_step == self._start_trace_step + int(
            _MLPERF_GPT3_TRACE_STEP.value
        ):
          logging.info('[trace]: Stop trace at step=%d', train_step)
